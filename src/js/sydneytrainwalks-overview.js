...
SydneyTrainWalks.prototype.Overview = function(parent) {
  ...
  this.getMarkers = function() {
    var route, photo, index, totNum, totLat, totLon, markers = [],
      prefix, start, end;
    // for every walk
    for (route in GuideData) {
      // get the source of the assets
      prefix = (GuideData[route].assets) ? GuideData[route].assets.prefix : route;
      start = (GuideData[route].assets) ? GuideData[route].assets.start : -1;
      end = (GuideData[route].assets) ? GuideData[route].assets.end : 9999;
