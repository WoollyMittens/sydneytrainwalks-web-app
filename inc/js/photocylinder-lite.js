import { Popup } from "./photocylinder-popup-lite.js";
import { Stage } from "./photocylinder-stage-lite.js";
import { Busy } from "./photocylinder-busy-lite.js";
import { Fallback } from "./photocylinder-fallback-lite.js";
...
export class Photocylinder {
	...
	success(url, fov) {
		...
		var isSpherical = this.config.spherical.test(url);
		...
		this.stage = (isSpherical || isWideEnough) ? new Stage(this) : new Fallback(this);
		this.stage.init();
		// trigger the success handler
		if (config.success) { config.success(config.popup); }
	}
